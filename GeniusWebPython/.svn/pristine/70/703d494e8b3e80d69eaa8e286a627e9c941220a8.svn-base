...
class ProtocolRefTest (unittest.TestCase) :
	pyson=ObjectMapper()
	
	
	#NOTICE #38. We work around it here
	ref=ProtocolRef(URI("http://local/"))
	ref1=ProtocolRef(URI("http://local/"))
	ref2=ProtocolRef(URI("http://host"))
	...
	def testDeserialize(self):
		self.assertEqual(self.ref, self.pyson.parse(self.refjson, ProtocolRef))
	
	def testRepr(self):
		self.assertEqual("ProtocolRef[http://local/]", repr(self.ref))
